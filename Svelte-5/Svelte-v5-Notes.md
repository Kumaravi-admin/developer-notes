...
Unlike React or Vue, Svelte does its work at compile time — converting your components into highly optimized vanilla JavaScript, rather than shipping a heavy runtime to the browser.
This means faster load times, smaller bundle sizes, and better performance.
...
| 3       | Runes : $state – II                                   | $state.raw, $state.snapshot, Passing State into Functions  |
| 4       | Runes : $state – III                                  | $state.eager, Sharing State Across Modules                 |
| 5       | Runes : $derived                                      | $derived, $derived.by                                      |
| 6       | Runes : $derived – II                                 | Deriveds and Reactivity, Destructuring, Update Propagation |
...
| 28      | Template syntax : {#snippet}, {@render}               | Snippet, Render                                            |
...
## Page 1 — Installation & Folder Structure
...
# Or install Svelte using Vite
npm create vite@latest

# 2. Move to project directory and run server
cd 1svelte
npm run dev
```
...
Components are the building blocks of Svelte applications. They are written in `.svelte` files, using a superset of HTML.
...
<script>
  let count = $state(0); // Create a reactive variable using $state Rune
</script>
...
<!-- Add a new todo -->
<button onclick={() => todos.push({ id: todos.length + 1, done: false, text: 'new todo' })}>Add Todo</button>
...
<!-- ✓ reassign — works -->
<button onclick="{()" ="">
  (person = { name: 'Ravi Kumar', age: 28 })}>Reassign person
</button>
...
Share reactive state across multiple components by declaring it in a `.svelte.js` file.
...
<script>
  ...
  import {
    ...
    decrement,
    reset,
  } from "$lib/components/counter.svelte.js";
</script>
...
Unlike `$effect`, it does not cause side effects and should be used whenever you need to compute something from existing state.
For complex expressions, `$derived.by` accepts a function instead of a simple expression.
...
`$derived.by` accepts a function as its argument. The function can contain multiple statements and can be as complex as needed.
The function will re-run whenever any of the dependencies change, and the result will be cached until the next change.
...
<button onclick="{()" ="">
  number.push(number.length + 1)}> {number.join(' + ')} = {total}
</button>
```

---

## Page 6 — Runes : $derived - II

### Derived : Deriveds and Reactivity
...
<script>
  ...
  // large is a derived variable that depends on count. It will be true if count is greater than 10,
...
## Page 7 — Runes : $derived - III

### Derived : untrack

Everything read synchronously inside `$derived` is tracked as a dependency. When any dependency changes, derived recalculates.
...
Effects are functions that automatically run when reactive state changes, and only execute in the browser — never during server-side rendering. Svelte tracks which reactive values are read inside an effect and re-Runes it when they change.
Multiple state changes are batched together, causing only one re-run. An effect can return a teardown function that Runes before the effect re-Runes or when the component is destroyed — useful for cleaning up intervals or event listeners. Use effects only for DOM manipulation, canvas drawing, or syncing with external libraries — never for synchronising state, that is what `$derived` is for.
...
Return a cleanup function — Runes before effect re-Runes or component destroys.
...
<script>
  ...
  // effect Runes once when the component is mounted or re-rendered when dependencies change
  $effect(() => {
...
## Page 9 — Runes : $effect - II

### Effect : $effect.pending

When using `await` in components, the `$effect.pending()` rune tells you how many promises are pending in the current boundary, not including child boundaries. `$effect.pending()` returns a number of pending promises; if no any pending promises, returns 0.
...
<!-- Add a new message -->
<button onclick="{()" ="">
  messages.push(`Message ${messages.length + 1}`)}>Add Message
</button>
```
...
## Page 11 — Runes : $effect - IV

### Effect : Understanding Dependencies

`$effect` automatically tracks reactive values (`$state`, `$derived`, `$props`) read synchronously inside it and re-Runes when they change.
...
<script>
  $effect(() => {
    context.fillStyle = color; // ✓ tracked — re-Runes when color changes
...
<!-- 2. Object vs Property
     Effect reRunes when object itself changes — not when a property inside it mutates. -->
<script>
  ...
  $effect(() => {
    state;
  }); // Runes once — object never reassigned
  $effect(() => {
    state.value;
  }); // Runes on every value change ✓
  $effect(() => {
    derived;
  }); // Runes every time — new object each time ✓
</script>
...
Create components folder in `src/lib/` — in component folder create `Mycomponent.svelte` file.
...
## Page 14 — Runes : $props (continued)

### Props : Default props values

Destructuring allows us to declare fallback values, which are used if the parent component does not set a given prop (or the value is undefined). In this example, the fallback value for `adjective` is `'Happy'`.

```html
<!-- src/lib/components/Mycomponent.svelte -->

// default value is used if the prop is not provided by the parent component let
{ adjective = 'Happy' } = $props();
```
...
// Rename the super prop to trouper, and give it a default value let { super:
trouper = 'lights are gonna find me' } = $props();
```
...
// Store the rest of the props in a variable called 'others' let { a, b, c,
...others } = $props();
```
...
## Page 16 — Runes : $props (continued)

### Props : Updating props – II

```html
<!-- 3. If the prop is a reactive state proxy, mutations will have an effect
        but you will see an ownership_invalid_mutation warning, because the component
        is mutating state that does not 'belong' to it: -->
...
<button onclick="{()" ="">
  { // will cause the count below to update, // but with a warning. Don't mutate
  objects you don't own! object.count += 1; }} > clicks: {object.count} |
</button>

<!-- 4. Has no effect if the fallback value is used
         The fallback value of a prop not declared with $bindable is left untouched —
         it is not turned into a reactive state proxy, meaning mutations will not cause updates: -->
...
<button onclick="{()" ="">
  { // has no effect if the fallback value is used object.count += 1; }} >
  clicks: {object.count}
</button>
```
...
## Page 18 — Runes : $inspect

### About : $inspect

`$inspect` is a development-only debugging tool that works like `console.log` but automatically re-Runes whenever its reactive arguments change.
...
Works like `console.log` but re-Runes whenever its argument changes.
...
## Page 20 — Template syntax : Basic Markup
...
## Page 21 — Template syntax : Events
...
<!-- This is a simple Svelte component that includes a button with an onclick event handler. When the button
     is clicked, it will log 'clicked' to the console. -->
...
## Page 22 — Template syntax : Text expressions & Comments
...
## Page 23 — Template syntax : {#if ...} Logic Block
...
## Page 24 — Template syntax : {#if ...} Logic Block - II
...
## Page 25 — Template syntax : {#each ...} Loop Block

### Loop Block : each

`{#each}` is Svelte's way to loop through arrays, objects, or iterables like Map and Set. It supports index tracking, destructuring, and an `{:else}` block that shows when the list is empty. Always provide a unique key so Svelte can efficiently add, move, or remove items without re-rendering the whole list.
...
<!-- 3. With keyed — always use for better performance
         A key uniquely identifies each list item — Svelte uses it to efficiently
         insert, move, and delete items. Use strings or numbers as keys for best results. -->
...
{#key reset}
<Count />
<!-- Render the Count component, it will reset when 'reset' changes -->
{/key}
...
<script>
  function fetchData() {
    // Fetch data from an API and return a promise
    let users = fetch("https://jsonplaceholder.typicode.com/posts/1").then(
      (res) => res.json(),
    );
    return users;
  }
...
<p>Loading...</p>
<!--while waiting for the promise to resolve, show a loading message -->
{:then data}
<!-- once the promise resolves, we can access the data -->
<p>{data.title}</p>
<p>{data.body}</p>
<!-- display the title and body from the fetched data -->
{:catch error}
<!-- if there is an error while fetching the data, catch it -->
<p>Error: {error}</p>
<!-- display the error message -->
{/await}

<!-- The catch block can be omitted if you don't need to render anything when the promise rejects -->
{#await promise}
<!-- promise is pending -->
...
## Page 28 — Template syntax : {#snippet ...}, {@render ...}

### Syntax : #snippet

Snippets, and render tags, are a way to create reusable chunks of markup inside your components.
Instead of writing duplicative code you can write this. To render a snippet, use a `{@render ...}` tag.

> **Note:** Like function declarations, snippets can have an arbitrary number of parameters, which can have default values, and you can destructure each parameter. You cannot use rest parameters, however.
...
{#each images as image (image.id)} {#if image.href}
<!--Check if the image has a link -->
<a href="{image.href}">
  <!-- If it does, wrap the figure in an anchor tag -->
  {@render figure(image)}
  <!-- To render a snippet, use a {@render ...} tag -->
</a>
{:else} {@render figure(image)}
<!-- 4 Otherwise, just render the figure -->
{/if} {/each}
...
## Page 29 — Template syntax : Passing snippets to components
...
## Page 30 — Template syntax : Passing snippets to components - II
...
## Page 31 — Template syntax : Passing snippets to components - III
...
## Page 32 — Template syntax : {@render ...}
...
{#if children} {@render children()} {:else} fallback content {/if}
```

---

## Page 33 — Template syntax : Snippet scope
...
<!-- To render a snippet, use a {@render ...} tag -->
{@render hello('svelte')} {@render hello('Anjesh')}

<!-- and they are 'visible' to everything in the same lexical scope (i.e. siblings,
     and children of those siblings): -->
<div>
  {#snippet x()} {#snippet y()}...{/snippet}
...
{#snippet blastoff()}
<span>🚀</span>
{/snippet} {#snippet countdown(n)} {#if n > 0}
<span>{n}...</span>
{@render countdown(n - 1)} {:else} {@render blastoff()} {/if} {/snippet}
{@render countdown(10)}
```

---

## Page 34 — Template syntax : {@html ...}, {@const ...}, {@debug ...}
...
> **Note:** `{@const}` is only allowed as an immediate child of a block — `{#if ...}`, `{#each ...}`, `{#snippet ...}` and so on — a `<Component />` or a `<svelte:boundary>`.
...
{#each boxes as box, i (i)} {@const area = box.width * box.height}
<p>{box.width} x {box.height} = <strong>{area}</strong></p>
{/each}
```
...
<!-- WON'T compile -->
{@debug user.firstname} {@debug myArray[0]} {@debug !isReady} {@debug typeof
user === 'object'}
```

---

## Page 35 — Template syntax : {@attach ...}
...
<script>
    ...
    function tooltip(text) {                               // Function to attach the tooltip
        return (node) => {                                 // Function to create the tooltip
...
<div {@attach (node)="">
  ...
  Animate the opacity from 0 to 1 over 1 second }} > ➤ I fade in!
</div>
```

---

## Page 36 — Template syntax : {@attach ...} - II
...
<!-- toggle focus -->
<button onclick="{()" ="">(isfocused = !isfocused)}>Toggel Focus</button>
```
...
<input type="text" bind:value="{" ()="" /> value, //get function (v) => value =
v.toLowerCase() //set function }/>
...
## Page 38 — Template syntax : bind: - II

### Bind : defaultChecked

Use `defaultChecked` to set the initial value without two-way binding:
...
<input type="checkbox" defaultChecked="{accepted}" />
```

### Bind : indeterminate

Checkboxes also have an `indeterminate` state — visually, a horizontal line rather than a tick or empty box:
...
`bind:value` on a `<select>` element binds to the `value` of the selected `<option>`:
...
<!-- If the initial value of selected doesn't match any option, the first option will be selected.
     A <select> element can have a multiple attribute, in which case it will populate an array
     rather than selecting a single value: -->
...
## Page 39 — Template syntax : bind: - III

### Bind : bind:files

`bind:files` on `<input type="file">` gives you a `FileList` of the selected files:
...
Inputs that work together can use `bind:group`. Radio inputs with the same `bind:group` value are mutually exclusive; checkbox inputs with the same `bind:group` value form an array of selected values.
...
<p>
  You ordered {scoops} {scoops === 1 ? 'scoop' : 'scoops'} of {flavours.join(',
  ')}
</p>
```
...
## Page 40 — Template syntax : bind: - IV

### Bind : Audio & Image

Svelte has `bind:` support for media elements including `<audio>` and `<img>`.

**Image bind:naturalWidth / bind:naturalHeight** — get the intrinsic size of an image:
...
**Audio bindings** — audio elements support several bindable properties:
...
<button onclick="{()" ="">
  paused = !paused}> {paused ? 'Play' : 'Pause'}
</button>
<p>Time: {currentTime.toFixed(1)} / {duration.toFixed(1)}s</p>
...
<form>
  ...
  {#if indeterminate} waiting... {:else if checked} checked {:else} unchecked
  {/if}
</form>
```
...
<!-- reset the files chosen (choose button) -->
...
<label
  ><input type="radio" bind:group="{tortilla}" value="Plain" /> Plain</label
>
<label
  ><input type="radio" bind:group="{tortilla}" value="Whole wheat" /> Whole
  wheat</label
>
<label
  ><input type="radio" bind:group="{tortilla}" value="Spinach" /> Spinach</label
>

<!-- grouped checkbox inputs populate an array -->
<label
  ><input type="checkbox" bind:group="{fillings}" value="Rice" /> Rice</label
>
<label
  ><input type="checkbox" bind:group="{fillings}" value="Beans" /> Beans</label
>
<label
...
>
<label
  ><input type="checkbox" bind:group="{fillings}" value="Guac (extra)" /> Guac
  (extra)</label
>
...
## Page 41 — Template syntax : bind: - V
...
In this example, demonstrate two way binding (child <----> parent) with a custom Input component.
...
## Page 42 — Template syntax : bind: - VI
...
## Page 43 — Template syntax : bind: - VII
...
<script>
  ...
  let devicePixelContentBoxSize = $state(null);
</script>
...
<p>
  contentRect: {contentRect?.width.toFixed(0)} x
  {contentRect?.height.toFixed(0)}
</p>
...
contentRect  = ResizeObserver (most accurate)
```

---

## Page 44 — Template syntax : Await
...
<script>
  // thrown error will be catched by failed block
...
<svelte:boundary>
  {#snippet pending()}
  <!-- Render the pending blocks -->
  <p>Loading...</p>
  {/snippet} {#snippet failed(error)}
  <!-- Render the failed block -->
  <p>Error : {error.message}</p>
  {/snippet}

  <p>{await fetchData()}</p>
  <!-- Render the success block -->
</svelte:boundary>
```

---

## Page 45 — Template syntax : Await - II

### Await Indicating loading states : pending snippet

1. `pending` snippet — first load only. Shown when the boundary first creates. Disappears once async content resolves. Never shown again on subsequent updates.
...
<!-- Using await & svelte:boundary to handle the promise state -->
<svelte:boundary>
  {#snippet pending()}
  <!-- This block will be shown while the promise is pending -->
  <p>Loading... please wait</p>
  {/snippet}

  <p>{await fetchData()}</p>
  <!-- This block will be shown when the promise resolves -->
</svelte:boundary>
```

### Await Indicating loading states : $effect.pending()

`$effect.pending()` is a function that returns true when async work is in progress — for subsequent updates after the first load. Used to show a spinner or loading indicator.
...
<svelte:boundary>
  <!-- check promises resolved or pending with $effect.pending -->

  {#if $effect.pending()}
  <!-- check if a promise is pending -->
  <p>checking...</p>
  {:else}
  <span>{await validate(username)}</span>
  <!-- show validation result when promise is resolved -->
  {/if}
</svelte:boundary>
```
...
## Page 46 — Template syntax : Await - III
...
Independent `await` expressions in markup run in parallel automatically. However, sequential `await` inside `<script>` or async functions run one after another like normal JavaScript. Independent `$derived` expressions update independently after their first creation, even if created sequentially. Markup — runs in parallel.
...
## Page 47 — Template syntax : Await - IV
...
<svelte:boundary>
  ...
  <p>Loading data...</p>
  {/snippet} {#snippet failed(error)}
  <!-- show if fetchData throws an error -->
  <p>Error: {error.message}</p>
  {/snippet}
...
## Page 48 — Template syntax : Transition
...
<!-- fade in and out -->
{#if visible}
    <div transition:fade={{ duration: 200 }}>fade in and out over two seconds</div>
{/if}
```

### Transition : Transition events

An element with transitions will dispatch the following events in addition to any standard DOM events: `introstart`, `introend`, `outrostart`, `outroend`.
...
## Page 49 — Template syntax : Transition - II
...
## Page 50 — Template syntax : in: and out: & Built-in transitions
...
## Page 51 — Template syntax : Animation
...
Custom animation functions receive three arguments: `node` — the DOM element; `from`, `to` — element's start and end positions as DOMRects (x, y, width, height); any extra parameters passed to the animation. The `css` function runs repeatedly before animation starts, receiving `t` (0→1) and `u` (1-t). Svelte uses the returned CSS to create a web animation on the element.
...
<!-- Shuffle button -->
<button onclick="{()" ="">
  (list = [...list].sort(() => Math.random() - 0.5))}>Shuffle
</button>
...
## Page 52 — Template syntax : Class

### About : Class

There are two ways to set classes on elements: the `class` attribute and the `class:` directive.

- **class Attribute (Preferred)** — Accepts primitives, objects, and arrays. Objects apply truthy keys as classes. Arrays skip falsy values. Uses clsx internally. Composes cleanly across components. Available since Svelte 5.16.
- **ClassValue Type** — Since Svelte 5.19, import `ClassValue` from `svelte/elements` for type-safe class props in TypeScript.
...
## Page 53 — Template syntax : Class - II
...
<!-- Toggle the style -->
<button onclick="{()" ="">
  (useStyle = !useStyle)} class={{ 'my-custom-class': useStyle }}>Toggle Style
</button>
...
<div style:color>Hello Svelte Developer</div>
<!-- Short hand syntax -->
```
...
<div style:color="{Mycolor}">Hello Svelte Developer</div>
<!-- Short hand syntax -->
...
## Page 55 — Svelte Styling : Style - II
...
<div style:color="red" style="color: blue !important">
  This will still be read
</div>
```
...
<!-- dynamic styles -->
<div style:--primary={primaryColor} style:--size={fontSize + 'px'} class="box">Hello Svelte!</div>
...
## Page 56 — Svelte Styling : Style - III
...
- Define → `@keyframes -global-fadebounce` (with `-global-`)
...
## Page 57 — Svelte Styling : Style - IV
...
### Style : Custom properties on components
...
## Page 58 — Svelte Styling : Style - V

### Style : Nested `<style>` elements

Only one top-level `<style>` allowed per component. Nested `<style>` inside elements is possible,
but it's inserted into the DOM as-is — no scoping, no processing — so it affects every matching element in the entire app.
...
## Page 59 — Svelte Special elements : svelte:boundary
...
<svelte:boundary>
  {#snippet pending()}
  <!-- show while fetchData is resolving pending state -->
  <p>Loading data...</p>
...
## Page 60 — Svelte Special elements : svelte:boundary - II
...
<p>Error: {error.message}</p>
<button onclick="{()" ="">
  { error = null; // clear the error reset(); // call reset to reset the
  boundary and try again }}>Try again
</button>
{/if}
```
...
## Page 61 — Svelte Special elements : svelte:window
...
```html
<svelte:window onevent="{handler}" /> <svelte:window bind:prop="{value}" />
```
...
## Page 62 — Svelte Special elements : svelte:document
...
`<svelte:document>` — Bind `visibilityState`. Bind `visibilityState` to reactively track whether the page is visible or hidden. Returns "visible" when active and "hidden" when tab is switched or window is minimized.
...
## Page 63 — Svelte Special elements : svelte:body, svelte:head
...
## Page 64 — Svelte Special elements : svelte:element, svelte:options

### Special element : `<svelte:element>`
...
### Special element : `<svelte:options>`

The `<svelte:options>` element provides a place to specify per-component compiler options. The possible options are:
...
- `customElement={...}` — the options to use when compiling this component as a custom element. If a string is passed, `css="injected"` — the component will inject its styles inline. During server-side rendering, it's injected as a `<style>` tag in the head; during client side rendering, it's loaded via JavaScript.
...
## Page 65 — Svelte Runtime : createContext

### Context : createContextbe
...
## Page 66 — Svelte Runtime : Lifecycle hooks
...
Does not run on SSR. If a function is returned, it Runes on unmount.
...
Out of `onMount`, `beforeUpdate`, `afterUpdate` and `onDestroy`, this is the only one that Runes inside a server-side component.
...
// ✓ normal reactive state let count = $state(0); // ✓ large object — only
reassigned, use $state.raw let apiResponse = $state.raw({}); apiResponse = await
...
// ✓ do this let square = $derived(num * num); // ✗ don't do this let square;
...
let { type } = $props(); // ✓ do this let color = $derived(type === 'danger' ?
'red' : 'green'); // ✗ don't do this — won't update if type changes let color =
...
## Page 68 — Svelte : Best practices - II
...
// ✓ do this import { createContext } from 'svelte'; export const [getUser,
setUser] = createContext(); // ✗ don't do this import { setContext, getContext }
from 'svelte'; setContext('user', user);
```
